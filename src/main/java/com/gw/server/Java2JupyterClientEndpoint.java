...
public class Java2JupyterClientEndpoint extends Endpoint {
  ...
  public void init(
    ...
    try {
            ...
            public void beforeRequest(Map<String, List<String>> nativeheaders) {
              ...
              while (hmIterator.hasNext()) {
                ...
                if ("Host".equals(newkey)
                    || "Origin".equals(newkey)
                    || "Sec-WebSocket-Key".equals(newkey)) {
                ...
                //                        	local.add(hostjupyters[0] + "://" + hostjupyters[1] + ":"
                // + hostjupyters[2]);
                //                        	uppercaseheaders.put(newkey, local);
                //
                //                        }else {
      ...
      //            ClientEndpointConfig.Configurator configurator = new
      ...
      //            clientConfig.getUserProperties().put("Sec-WebSocket-Extensions",
      // "permessage-deflate; client_max_window_bits");
      //            clientConfig.getUserProperties().put("Sec-WebSocket-Key",
      // "JNOrKMA6YhDCRijp46/ofg==");
      ...
      logger.debug(
          "Default Binary Max Message Buffer Size: "
  ...
  public void onOpen(Session session, EndpointConfig config) {
    ...
    //		                	}else {
    //
    //		                		logger.warn("The websocket between browser and geoweaver is null or
    // closed");
    //
    //		                	}
    //
    ...
    ////
    ////		                	}
    //		//                    session.getBasicRemote().sendText("Got message from " +
    // session.getId() + "\n" + message);
    //                		} catch (Exception ex) {
    //                			ex.printStackTrace();
    //                        	logger.error("Fail to parse the returned message from Jupyter server"
  ...
  public void onClose(Session session, CloseReason closeReason) {
    try {
      ...
      if (!BaseTool.isNull(pair)) {
        ...
        pair.getGeoweaver_jupyter_client()
  ...
  public void setNew_ws_session_between_geoweaver_and_jupyterserver(
    ...
    this.new_ws_session_between_geoweaver_and_jupyterserver =
        new_ws_session_between_geoweaver_and_jupyterserver;
  }
  //
  ...
  //	}
  //
  //	public void setNew_ws_session_between_browser_and_geoweaver(Session
  // new_ws_session_between_browser_and_geoweaver) {
  //		this.new_ws_session_between_browser_and_geoweaver =
  // new_ws_session_between_browser_and_geoweaver;
  //	}
  ...
  ////    	if(!BaseTool.isNull(this.newjupyteression))
