...
@ServerEndpoint(
    value = "/jupyter-socket/{hostid}/api/kernels/{uuid1}/channels",
...
public class JupyterRedirectServlet {
  ...
  // 			System.out.println("Detected one session pair is closed on one of the websocket session.
  // Removing it...");
  ...
  public void open(
    ...
    try {
      ...
      Map<String, List<String>> headers =
          (Map<String, List<String>>) config.getUserProperties().get("RequestHeaders");
      ...
      //			logger.debug("The connections from javascript end to this servlet, and this servlet to
      // Jupyter server have been created.");
  ...
  public void echo(String message, @PathParam("uuid1") String uuid1, Session session) {
    ...
    try {
      ...
      if (BaseTool.isNull(pair)) {
        ...
        //
        //	        	logger.debug("UUID string: " + uuid1 + " - Session ID: " +
        // session.getQueryString());
