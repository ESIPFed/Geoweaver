...
public class TestSocketServlet {
  ...
  public void echo(String message, Session session) {
    ...
    try {
      ...
      //        	session.getBasicRemote().sendText("Message received and Geoweaver Shell Socket Send
      // back: " + message);
      ...
      //                // Can the client send the websocket session id and username in a REST call
      // to link them up?
      //                sshSession =
      // GeoweaverController.sessionManager.sshSessionByToken.get(message);
      //
      ...
      //
      //                	GeoweaverController.sessionManager.sshSessionByToken.put(session.getId(),
      // sshSession);
      //
      ////                	GeoweaverController.sessionManager.sshSessionByToken.remove(messageText);
      // //remove session, a token can only be used once
      //
      //                }else {
      //
      //                	if(session.isOpen()) {
      ...
      //                	sshSession.logout();
      //
      ////                	session.close(); //close WebSocket session. Notice: the SSHSession will
      // continue to run.
      //
      //                }
      //
  ...
  public void close(final Session session) {
    ...
    try {
      ...
      //            	SSHSession sshSession =
      // GeoweaverController.sessionManager.sshSessionByToken.get(session.getId());
      //                if (sshSession != null && sshSession.isTerminal()) { //only close when it is
      // shell
      //                    sshSession.logout();
      //                }
      //
