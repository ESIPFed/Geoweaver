...
public class TerminalServlet {
  ...
  public void echo(String message, Session session) {
    ...
    try {
      ...
      SSHSession sshSession =
          GeoweaverController.sessionManager.sshSessionByToken.get(httpSessionID);
      ...
      if (sshSession == null) {
        ...
        if (sshSession != null) {
          ...
          //
          //	GeoweaverController.sessionManager.sshSessionByToken.remove(messageText); //remove
          // session, a token can only be used once
  ...
  public static jakarta.websocket.Session findSessionById(String sessionid) {
