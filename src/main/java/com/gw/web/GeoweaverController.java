 ...
 * handles various web requests related to Geoweaver's functionality.
 */
@Controller
...
public class GeoweaverController {
  ...
  /**
   * Handles a POST request to delete a resource (host, process, workflow, history, or clear nodes
   * and edges).
   *
   * @param model The ModelMap to store response data.
   * @param request The WebRequest containing request parameters.
  ...
  public @ResponseBody String listhostwithenvironments(
   ...
   * @param session The user's HTTP session.
   * @param httprequest The HTTP request.
   * @return A JSON response containing a list of resources of the specified type or an error
   *     message.
   */
  ...
  public @ResponseBody String list(
    ...
    try {
      ...
      // Retrieve the authenticated user's ownerid based on the session and HTTP request IP.
      String ownerid = ut.getAuthUserId(session.getId(), ut.getClientIp(httprequest));
      // Depending on the resource type, retrieve a list of resources owned by the authenticated
      // user.
      if (type.equals("host")) {
        resp = ht.list(ownerid);
      } else if (type.equals("process")) {
  ...
  public @ResponseBody String closefileBrowser(
      ModelMap model, WebRequest request, HttpSession session) {
    String resp = null;
    try {
  ...
  /**
   * Handles HTTP POST requests to open the file browser, either initializing a new session or
  ...
  public ResponseEntity<Resource> fileGetter(
      ModelMap model,
      @PathVariable(value = "tempfolder") String tempfolder,
  ...
  public @ResponseBody String executeWorkflow(
      ModelMap model, WebRequest request, HttpSession session) {
    String resp = null;
    try {
      String id = request.getParameter("id");
      ...
      String history_id =
          BaseTool.isNull(request.getParameter("history_id"))
              ? new RandomString(18).nextString()
              : request.getParameter("history_id");
  ...
  public @ResponseBody String addLocalFile(
      ModelMap model, WebRequest request, HttpSession session) {
  ...
  public @ResponseBody String executeProcess(
      ModelMap model, WebRequest request, HttpSession session) {
    String resp = null;
    try {
      ...
      String history_id =
          BaseTool.isNull(request.getParameter("history_id"))
              ? new RandomString(12).nextString()
              : request.getParameter("history_id");
  ...
  public @ResponseBody String editprocess(
      ModelMap model, @RequestBody GWProcess up, WebRequest request) {
    String resp = null;
    try {
      checkID(up.getId());
      pt.save(up);
  ...
  public @ResponseBody String editworkflow(
      ModelMap model, @RequestBody Workflow w, WebRequest request) {
    String resp = null;
    try {
      checkID(w.getId());
      wt.save(w);
  ...
  public @ResponseBody String addProcess(
      ModelMap model, @RequestBody GWProcess np, WebRequest request) {
    String resp = null;
    try {
  ...
  public @ResponseBody String addWorkflow(
      ModelMap model, @RequestBody Workflow w, WebRequest request) {
    String resp = null;
    try {
  ...
  public @ResponseBody String add(ModelMap model, WebRequest request) {
    ...
    try {
      ...
      if (type.equals("host")) {
        ...
        String confidential = request.getParameter("confidential");
        String ownerid =
            BaseTool.isNull(request.getParameter("ownerid"))
                ? "111111"
                : request.getParameter("ownerid");
        String hostid =
            ht.add(hostname, hostip, hostport, username, url, hosttype, ownerid, confidential);
        resp =
            "{ \"id\" : \""
                + hostid
                + "\", \"name\" : \""
        ...
        if (lang.equals("shell")) {
          ...
          while (request.getParameter("code[params][" + i + "][name]") != null) {
            ...
            code +=
                "{ \"name\": \""
                    + request.getParameter("code[params][" + i + "][name]")
                    + "\", \"value\": \""
                    + request.getParameter("code[params][" + i + "][value]")
                    + "\" }";
            i++;
          }
          code += "] }";
  ...
  public String sshterminal(
      ModelMap model, WebRequest request, SessionStatus status, HttpSession session) {
    String token = request.getParameter("token");
  ...
  public @ResponseBody String ssh_close_inbox(
      Model model, WebRequest request, HttpSession session) {
    String resp = "";
    try {
      String token = request.getParameter("token");
      if (token != null) {
