...
public class CheckpointController {
  ...
  @Autowired private CheckpointTool checkpointTool;
  ...
  public ResponseEntity<List<CheckpointDTO>> getAllCheckpoints(@PathVariable String workflowId) {
    ...
    List<CheckpointDTO> checkpointDTOs =
        checkpoints.stream().map(checkpointTool::convertToDTO).collect(Collectors.toList());
    return ResponseEntity.ok(checkpointDTOs);
  }

  @PostMapping("/create")
  public ResponseEntity<Checkpoint> createCheckpoint(
      @RequestBody CheckpointCreateRequest createRequest) {
    try {
  ...
  public ResponseEntity<Checkpoint> restoreWorkflow(@RequestBody CheckpointRestoreDTO restoreDTO) {
    try {
      Checkpoint checkpointRestore =
          checkpointTool.restoreCheckpoint(restoreDTO.getWorkflowId(), restoreDTO.getExecutionId());
