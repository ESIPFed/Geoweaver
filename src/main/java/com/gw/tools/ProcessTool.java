...
public class ProcessTool {
   ...
   *
   * @param owner
   * @param isactive If this is true, only return the processes that are still running.
   * @return
   * @throws SQLException
   */
  ...
  public String detail(String id) throws JsonProcessingException {
    ...
    if (!BaseTool.isNull(p.getCode())
        && (p.getCode().contains("bash\\\n")
            || p.getCode().contains("\\\nimport")
            || p.getCode().contains("\\\"operation\\\"")
            || p.getCode().contains("\\\"cells\\\""))) {
  ...
  public String add_database(
      String name, String lang, String code, String desc, String ownerid, String confidential) {
  ...
  public String getCodeById(String pid) {
    ...
    if (code.contains("bash\\\n")
        || code.contains("\\\nimport")
        || code.contains("\\\"operation\\\"")
        || code.contains("\\\"cells\\\"")) code = this.unescape(code);
