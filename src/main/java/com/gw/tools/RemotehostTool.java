...
public class RemotehostTool {
  ...
  public String executeShell(
    ...
    try {
      ...
      //			GeoweaverController.sessionManager.closeWebSocketByToken(token); //close this websocket
      // at the end
   ...
   * @param hid
   * @param pswd
   * @param token for security reasons
   * @param isjoin
   * @return
   */
  public String executePythonProcess(
    ...
    try {
      ...
      // GeoweaverController.sessionManager.closeWebSocketByToken(token); //close this websocket at
      // the end - don't close - the websocket channel should stay on
