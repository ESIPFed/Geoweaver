...
/**
 * This class represents the implementation of a local session for Linux and Mac. It provides
 * methods for running commands and scripts on the local system.
 *
 * @author JensenSun
 */
@Service
...
public class LocalSessionNixImpl implements LocalSession {
   ...
   * @param isjoin A boolean flag indicating whether to wait for the script execution to complete
   *     (join).
  ...
  public void runBash(
    ...
    try {
      ...
      // Initialize the sender with the input stream, HTTP session token, history ID, execution type
      // (shell), and a null value for the script.
