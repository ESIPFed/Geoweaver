...
GW.workflow = {
  ...
  display: function (msg) {
    ...
    jQuery.each(msg, function (i, val) {
      ...
      if (i === "id") {
        ...
        id_combo =
          ...
          i +
          `</div>
					<div class="col col-md-3" id="display_workflow_id">` +
          val +
          `</div>`;
      } else if (i === "name") {
        workFlowName = val;
        ...
        desc_combo =
          ...
          `</div>
					...
					id="display_workflow_description_field" >` +
          val +
          `</textarea ></div>`;
    ...
    $("#display_workflow_name_field").focus(() => {
      current_workflow_name = $("#display_workflow_name_field").val();
    });
    ...
    $("#display_workflow_description_field").focus(() => {
      current_workflow_description = $(
        "#display_workflow_description_field",
      ).val();
    });
    ...
    $("#display_workflow_description_field").focusout(() => {
      if (
        current_workflow_description !=
        $("#display_workflow_description_field").val()
      ) {
        GW.workflow.updateWorkflowMetadata(); //if name changes, update the metadata
      }
    });
    ...
    switchTab(
      document.getElementById("main-workflow-info-code-tab"),
      "main-workflow-info-code",
    );
  ...
  newDialog: function (createandrun) {
    ...
    if (GW.workspace.checkIfWorkflow()) {
      ...
      if (GW.user.current_userid != null && GW.user.current_userid != "111111")
        content +=
          '       <input type="radio" name="confidential_new" value="TRUE"> ' +
      ...
      GW.workflow.new_frame = GW.process.createJSFrameDialog(
        520,
        450,
        content,
        "Authorization",
      ...
      $("#new-workflow-confirm").click(function () {
        ...
        if (
          typeof $('input[name="confidential_new"]:checked').val() !=
          "undefined"
        ) {
  ...
  save: function (nodes, edges) {
    if (this.loaded_workflow != null) {
      ...
      if (
        typeof $('input[name="confidential_workflow"]:checked').val() !=
        "undefined"
      ) {
    ...
    if (phs == null) {
      ...
      $.ajax({
        ...
        .done(function (msg) {
          ...
          var onehost =
            '   <div class="form-group row required" id="hostselectlist">' +
          ...
          $("input[name='modeswitch']").change(function (e) {
            ...
            if ($(this).val() == "one") {
              ...
              for (var i = 0; i < nodes.length; i++) {
                $("#selectarea").append(
                  '   <div class="form-group row required" id="hostselectlist_' +
                    i +
                    '">' +
                    ...
                    "</mark> on: </label>" +
                    '     <div class="col-sm-4">' +
                    '		<select class="form-control hostselector" id="hostforprocess_' +
                    i +
                    '">' +
                    "  		</select>" +
                    "     </div>" +
                    '     <div class="col-sm-4">' +
                    '		<select class="form-control environmentselector" id="environmentforprocess_' +
                    i +
                    '">' +
                    "  		</select>" +
                    "     </div>" +
            ...
            if ($("input[name=modeswitch]:checked").val() == "one") {
              ...
              for (var i = 0; i < nodes.length; i++) {
                hosts.push({
                  ...
                  id: $("#hostforprocess_" + i)
                    .find("option:selected")
                    .attr("id"),
                  env: $("#environmentforprocess_" + i)
                    .find("option:selected")
                    .attr("id"),
                });
              }
  ...
  restoreCheckpoint: function (workflowId, executionId) {
    if (
      confirm(
        "CAUTION: Restore will remove changes that are not stored in history. Proceed?",
      )
    ) {
      $.ajax({
  ...
  history: function (wid, name) {
    $.ajax({
      ...
      .done(function (msg) {
        ...
        GW.workflow.switchTab(
          document.getElementById("main-workflow-info-history-tab"),
          "main-workflow-info-history",
        );
      })
      .fail(function (jxr, status) {
  ...
  skipprocess: function (workflow_history_id, process_id) {
    var is_skipped = document.getElementById(
      "prompt_panel_skip_process_" + process_id,
    ).checked;
