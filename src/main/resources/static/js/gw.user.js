...
GW.user = {
  ...
  logindialog: function () {
    ...
    var content =
      '<div class="modal-body" style="font-size: 12px;" id="login_dialog_body">' +
      '<div class="row">' +
      '   <div class="col-md-12">' +
    ...
    this.login_frame = GW.process.createJSFrameDialog(
      300,
      400,
      content,
      "Login",
  ...
  refreshthetopbar: function (name, id, isloggedin) {
    if (isloggedin) {
      $("#login_dialog_body").html(
        '<div class="row"><div class="col-md-12"><h3>You have logged in successfully!</h3><p>This window will automatically close in 3 seconds..</p></div></div>',
      );
          ...
          $.ajax({
            ...
            .done(function (msg) {
              ...
              if (msg.status == "success") {
                ...
                $("#login_dialog_body").html(
                  '<div class="row"><div class="col-md-12"><h3 class="text-danger">Failed to log in. </h3><p>' +
                    msg.message +
                    '</p><p><a href="javascript:GW.user.logindialog()">Try Again</a></p></div></div>',
                );
              }
            })
  ...
  precheck: function (type) {
    ...
    if (type == "reset") {
      ...
      if (!isvalid) {
        $("#server_response_msg").html(
          "Either usename or password is missing. ",
        );
      }
    } else if (type == "signup") {
      ...
      if (!$("#username").val()) {
        ...
        $("#server_response_msg").html(
          "You have to agree the terms to register an account",
        );
      }
    }
  ...
  showProfileContent: function (message) {
    ...
    var content =
      `<div class="modal-body" style="font-size: 12px;" id="profile_dialog_body">
            <div class="row">
            <div class="col-md-12">
    ...
    this.profile_frame = GW.process.createJSFrameDialog(
      300,
      400,
      content,
      "Profile",
  ...
  closeOtherFrames: function () {
    try {
      ...
      if (GW.user.forget_password_frame != null)
        GW.user.forget_password_frame.closeFrame();

      if (GW.user.profile_frame != null) GW.user.profile_frame.closeFrame();

      GW.user.login_frame =
        GW.user.signup_frame =
        GW.user.forget_password_frame =
        GW.user.profile_frame =
          null;
    } catch (error) {
      console.error(error);
    }
  ...
  signupdialog: function () {
    ...
    this.signup_frame = GW.process.createJSFrameDialog(
      400,
      400,
      content,
      "Registration",
        ...
        .done(function (msg) {
          ...
          if (msg.status == "success") {
            $("#reset-password-form").html(
              "<h3>A password reset email has been sent to you!</h3> <p>Please check your email to set new passwords.</p>" +
  ...
  forgetpassdialog: function () {
    ...
    var content =
      '<div class="modal-body" style="font-size: 12px;" id="reset-password-form">' +
      '   <div class="row">' +
      '      <div class="col-md-12">' +
    ...
    this.forget_password_frame = GW.process.createJSFrameDialog(
      400,
      400,
      content,
      "Forget Password",
