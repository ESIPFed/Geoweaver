GW.filebrowser = {
  ...
  openFileEditor: function (file_name) {
    $.ajax({
      ...
      if (msg.ret == "success") {
        ...
        var content =
          '<div class="modal-body" style="font-size:12px;" ><div id="codearea" class="form-group row required" ></div>' +
        ...
        $("#codearea").append(
          '<textarea id="code_editor" placeholder=""></textarea>',
        );

        GW.filebrowser.editor = CodeMirror.fromTextArea(
        ...
        $("#browser-run").click(function () {
          ...
          if (GW.filebrowser.isIn(suffix[1], ["py", "sh"])) {
            ...
            $.ajax({
              ...
              GW.ssh.addlog(
                "The process " + msg.name + " is added to the process list.",
              );
              GW.ssh.addlog(
                "Pop up authorization dialog to initiate the run of the process : " +
                  pid,
              );
            });
          } else {
  ...
  operatefile: function (file_name, file_size) {
    ...
    if (suffix !== null) {
      if (
        Number(file_size) < 10 * 1024 * 1024 &&
        GW.filebrowser.isIn(suffix[1], [
          "txt",
          "py",
          "sh",
          "java",
  ...
  continuebrowser: function (file_name) {
    $.ajax({
      ...
      .done(function (msg) {
        ...
        if (msg.ret != "failure") GW.filebrowser.updateBrowser(msg);
        else alert("Fail to open the directory: " + msg.msg);
      })
      .fail(function (error) {
  ...
  updateBrowser: function (msg) {
    ...
    if (GW.filebrowser.current_path == "/") parentfolder = ".";
    ...
    for (var i = 0; i < msg.array.length; i++) {
      ...
      if (msg.array[i].isdirectory) {
        cont +=
          '    <td class="col-md-6 word-wrap" ><span><i class="pull-left fa fa-folder" style="padding-right: 5px;"></i><a class="word-wrap" href="javascript:GW.filebrowser.continuebrowser(\'' +
          msg.array[i].name +
          "')\" >" +
          msg.array[i].name +
          "</a></span></td>";
        ...
        cont +=
          '    <td class="col-md-5 word-wrap"><span><i class="pull-left fa fa-file" style="padding-right: 5px;"></i><a  class="word-wrap" href="javascript:GW.filebrowser.operatefile(\'' +
          msg.array[i].name +
          "', '" +
          msg.array[i].size +
          "')\" >" +
      ...
      cont +=
        "    <td>" +
        GW.filebrowser.s2Date(msg.array[i].mtime) +
        "</td>" +
        "    <td>" +
        msg.array[i].size +
        "</td>" +
  ...
  showFolderBrowserDialog: function (msg) {
    ...
    cont +=
      '<table class="table table-sm table-dark col-md-12" id="directory_table"> ' +
      "  <thead> " +
      "    <tr> " +
