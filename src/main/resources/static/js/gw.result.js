GW.result = {
  ...
  showDialog: function (process_history_id) {
    ...
    content +=
      ...
      '	<button type="button" id="result-download-' +
      dialogid +
      '" class="btn btn-outline-primary">Download</button> ' +
      '	<button type="button" id="result-preview-' +
      dialogid +
      '" class="btn btn-outline-secondary">Preview</button>' +
      '	<button type="button" id="result-cancel-' +
      dialogid +
    ...
    $.ajax({
      ...
      .done(function (msg) {
        ...
        $("#hostselector-" + dialogid)
          .find("option")
          .remove()
          .end();

        for (var i = 0; i < msg.length; i++) {
          $("#hostselector-" + dialogid).append(
    ...
    $("#result-preview-" + dialogid).click(function () {
      var hostid = $("#hostselector-" + dialogid)
        .children(":selected")
        .attr("id");
