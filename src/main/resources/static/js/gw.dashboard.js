...
GW.board = {
    ...
    $.ajax({
      ...
      .done(function (msg) {
        ...
        GW.board.real_time_status_chart.data.datasets[0].data = [
          msg.running_process_num,
          msg.failed_process_num,
          msg.success_process_num,
          msg.running_workflow_num,
          msg.failed_workflow_num,
          msg.success_workflow_num,
        ];
        ...
        first_splitter = Number(first_splitter).toFixed();

        var second_splitter = ((max_value - min_value) * 2) / 3 + min_value;
        second_splitter = Number(second_splitter).toFixed();

        GW.board.time_cost_chart.data.labels = [
          min_value + " - " + first_splitter + " ms",
          first_splitter + " - " + second_splitter + " ms",
          second_splitter + " - " + max_value + " ms",
        ];

        GW.board.time_cost_chart.data.datasets[0].data =
          GW.board.calculateFrequency(
            time_costs,
            first_splitter,
            second_splitter,
          );
  ...
  renderRealTimeStatusChart: function () {
    var ctx = document
      .getElementById("real_time_status_canvas")
      .getContext("2d");
